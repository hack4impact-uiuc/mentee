...
def validate_role(role):
    ...
    try:
        role_int = int(role)
        valid_roles = [Account.ADMIN.value, Account.MENTOR.value, Account.MENTEE.value, 
                      Account.PARTNER.value, Account.GUEST.value, Account.SUPPORT.value, 
                      Account.HUB.value, Account.MODERATOR.value]
...
def validate_file_upload(file, allowed_extensions=None, max_size_mb=50):
    ...
    if allowed_extensions is None:
        allowed_extensions = {'.pdf', '.doc', '.docx', '.txt', '.csv', '.jpg', '.jpeg', '.png', '.gif'}
    else:
    ...
    try:
        ...
        if expected_mime and not mime_type.startswith(expected_mime.split('/')[0]) and mime_type != expected_mime:
            if not (file_ext in ['.csv', '.txt'] and mime_type in ['text/plain', 'text/csv', 'application/csv']):
                return False, "File content does not match extension"
        
        suspicious_patterns = [b'<script', b'javascript:', b'vbscript:', b'<?php', b'<%', b'\x4d\x5a']
