...
def get_eight_x_eight_config():
    ...
    return {
        ...
        'private_key': SecureEnvironmentManager.get_optional_env("EIGHT_X_EIGHT_ENCODED_PRIVATE_KEY")
    }
...
def generateToken():
    try:
        ...
        if not all([config['api_key'], config['app_id'], config['private_key']]):
            return create_response(status=500, message="8x8 configuration not available")
        ...
        return create_response(data={"token": token.decode("utf-8"), "appID": config['app_id']})
